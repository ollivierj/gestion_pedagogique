...
public class UtilisateurServiceImpl extends
		AServiceImpl<Utilisateur, Integer, UtilisateurDao> implements
		UtilisateurService {
	 ...
	 */
	@Inject
	public UtilisateurServiceImpl(UtilisateurDao pUtilisateurDao,
			ProfilDao profilDao, DroitProfilDao droitProfilDao)
	...
	public Utilisateur authentifier(String pLogin, String pMotDePasse)
			throws ApplicationException {
		Utilisateur lUtilisateur = null;
		User userLDAP = null;
		Date lCurrentDate = DateUtils.addHours(new Date(),
				AuthentificationConfiguration.getExpirationTime());
		if (ApplicationConfiguration.DEV_MODE
				.equals(AuthentificationConfiguration.getApplicationMode())) {
			lUtilisateur = this.dao.chargerDetail(AuthentificationConfiguration
					.getDefaultUser());
		...
		try {
			...
			Integer lUtilisateurId = this.dao.checkConnection(pLogin,
					pMotDePasse, (null != userLDAP));
			if (null != lUtilisateurId) {
				...
				// si l'authentification LDAP est ok et pas l'authentification
				// BDD alors créer l'utilisateur dans la BDD
				String[] userInfo = userLDAP.getCommonName().split(" ");
				Profil profilDefault = this.profilDao
						.chargerDetail(LDAPConfiguration.getDefaultUserProfil());
	...
	@Override
	public boolean checkConnection(String pLogin, String pMotDePasse,
	...
	protected String createToken(Utilisateur pUtilisateur, Date pDate) {
		...
		lStrBuilder.append(":");
		lStrBuilder
				.append(DateFormatUtils.format(pDate, "yyyy-MM-dd HH:mm:ss"));
