...
angular.module('schemaForm').provider('schemaForm',[function(){
  ...
  var stdFormObj = function(schema,options) {
    ...
    if (options.disabled === true || schema.disabled === true) f.disabled = true;
    ...
    if (schema.type === "object") {
      ...
      angular.forEach(schema.properties,function(v,k){
        ...
        if (options.ignore[path] !== true) {
          ...
          var disabled = schema.disabled && schema.disabled.indexOf(k) !== -1;
  ...
  var array = function(name,schema,options){
    ...
    if (schema.type === 'array') {
      ...
      var disabled = schema.disabled && schema.disabled.indexOf(options.path) !== -1;
        ...
        angular.forEach(schema.properties,function(v,k){
            if (ignore[k] !== true) {
              ...
              var def = defaultFormDefinition(k,v,{
                ...
                required: required,
                disabled: disabled//Is it required? (v4 json schema style)
              });
              if (def) {
                form.push(def);
              }
